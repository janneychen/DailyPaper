...
# S2_API_KEY 现在可选，不设置也能使用（但有速率限制）
...
def get_paper_recommendations():
    ...
    # 如果有 API Key 就添加，没有就使用无 Key 调用
    if S2_API_KEY:
        headers["x-api-key"] = S2_API_KEY
        print("使用 API Key 调用 Semantic Scholar")
    else:
        print("未设置 API Key，使用无 Key 调用（注意速率限制）")
    ...
    print(f"API 返回 {len(raw_papers)} 篇推荐论文")
    ...
    print(f"过滤后剩余 {len(unseen_papers)} 篇未读论文")
    ...
    print(f"选取前 {len(top_new_papers)} 篇最新论文")

    # 步骤 2：如果启用 LLM 总结，则批量获取 TLDR
    if ENABLE_LLM_SUMMARY and top_new_papers:
